...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($updatetitle != null && !(preg_match('/^[a-zA-Z0-9]{1,30}$/', $updatetitle))) {
        $result_arr[] = array("message" => "Invalid Post Title");
        echo json_encode($result_arr);
        exit;
    }
    if (ctype_space($updatetitle)) {
        $result_arr[] = array("message" => "Invalid Post Title");
        echo json_encode($result_arr);
        exit;
    }
    $updatedes = $conn->real_escape_string($_POST['Post_description']);
    if ($updatedes != null && !(preg_match('/^[a-zA-Z0-9 ]{1,30}$/', $updatedes))) {
        $result_arr[] = array("message" => "Invalid Post Description ");
        echo json_encode($result_arr);
        exit;
    }
    if (ctype_space($updatedes)) {
        $result_arr[] = array("message" => "Invalid Post Description ");
        echo json_encode($result_arr);
        exit;
    }
